...
check_for_update() {
  ...
  if curl -fsSL "$SCRIPT_URL" -o "$TMP_SCRIPT"; then
    ...
    echo -e "⚠️  Failed to check for updates (network issue?)."
  fi
  rm -f "$TMP_SCRIPT"
}
...
PROFILE=$(whiptail --title "🎯 VazeAP Setup Menu" --menu "Select profile or features to install:" 24 80 9 \
  ...
  "serveropt"   "🛡️ Server Optimization: Sysctl, SSH, TCP, DNS, GRUB" \
...
install_vscode() {
  ...
  sudo snap install code --classic || echo -e "⚠️ VS Code install failed"
  progress 50
  echo -e "\n🖥️ VS Code installed."
}
...
install_backup() {
  ...
  progress 40
  echo -e "⚙️ Initializing Borg repository (example)"
...
install_ultraperf() {
  ...
  echo -e "\n⚙️ (Optional) You can install XanMod kernel manually later"
  progress 100
