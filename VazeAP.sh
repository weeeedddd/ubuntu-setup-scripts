...
# Initialize flags (for “custom” or “all” we could ask additional questions, but here we’ll keep profiles as standalone)
...
# For a future “all” or “custom” option you could prompt here…
# In this version: profiles run only their own modules.
...
install_serveropt() {
  ...
  echo -e "?? Optimizing SSH (disable root login, max auth attempts)"
  sudo sed -i 's/^#*PermitRootLogin.*/PermitRootLogin no/' /etc/ssh/sshd_config
  sudo sed -i 's/^#*MaxAuthTries.*/MaxAuthTries 3/' /etc/ssh/sshd_config
  sudo systemctl restart sshd
  progress 50

  echo -e "?? DNS tuning: installing resolvconf & setting Google DNS"
...
EOF'
  ...
  echo -e "?? GRUB tuning: choose profile"
  ...
  echo -e "?? Creating swap file and setting swappiness"
  SWAPSIZE=$(awk '/MemTotal/ {print $2/1024/1024*1}' /proc/meminfo | awk '{printf "%d", $1}')
  SWAPFILE="/swapfile"
  ...
  progress 100
  echo -e "\n?? Swap setup complete across $SWAPSIZE GB."
}
...
install_ultraperf() {
  ...
  sudo bash -c 'sed -i "s/defaults/defaults,noatime,nodiratime,commit=600/" /etc/fstab'
  progress 65

  echo -e "\n?? Reserving HugePages"
  TOTAL_RAM_MB=$(awk '/MemTotal/ {print $2/1024}' /proc/meminfo | awk '{printf "%d", $1}')
  HUGEPAGES=$(($TOTAL_RAM_MB / 1024 / 2))   # etwa 50% der RAM/1024 in GB * 512MB pro HugePage
  ...
  echo 'numa=interleave=all' | sudo tee -a /etc/default/grub
  sudo update-grub
  progress 90
